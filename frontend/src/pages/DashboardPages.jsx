...
export function RemoteControlPage() {
  ...
  const sendControl = (command) => {
    if (!session?.id || session.approved_permissions?.remote_input !== true || !controlReady || screenState !== "live") {
      setError("Remote controls are still connecting. Wait until the live screen and control channel are ready.");
      return false;
    }
    setError("");
  ...
  const sendNavigation = (type) => sendControl({ type });

  const sendSwipe = (direction) => {
    const presets = {
      up: { startX: 0.5, startY: 0.78, endX: 0.5, endY: 0.22 },
      down: { startX: 0.5, startY: 0.22, endX: 0.5, endY: 0.78 },
      left: { startX: 0.82, startY: 0.5, endX: 0.18, endY: 0.5 },
      right: { startX: 0.18, startY: 0.5, endX: 0.82, endY: 0.5 },
    };
    const gesture = presets[direction];
    if (!gesture) return;
    sendControl({ type: "swipe", ...gesture, durationMs: 180 });
  };
